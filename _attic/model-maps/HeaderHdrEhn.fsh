...
* author 1..* Base "A.1.5 - Author (by whom the Hospital discharge report was/were authored). Multiple authors could be provided." """Author (by whom the Hospital discharge report was/were authored). Multiple authors could be provided."""
...
* attester 0..* Base "A.1.6 - Attester (multiple attesters could be provided)" """Attester (multiple attesters could be provided)"""
...
* attester.organization 1..1 Base "A.1.6.4 - Attester organisation" """The healthcare provider organisation information."""
* attester.dateTime 1..1 dateTime "A.1.6.5 - Approval date and time" """Date and time of the approval of the document by Attester."""
* legalAuthenticator 0..1 Base "A.1.7 - Legal authenticator (The person taking responsibility for the medical content of the document)" """Legal authenticator (The person taking responsibility for the medical content of the document)"""
...
* legalAuthenticator.organization 1..1 Base "A.1.7.4 - Legal authenticator organisation" """The healthcare provider organisation information."""
...
* documentMetadata 1..1 Base "A.1.8 - Document metadata" """Document metadata"""
* documentMetadata.identifier 1..1 Identifier "A.1.8.1 - Document ID" """Unique identifier of the document"""
* documentMetadata.type 1..1 CodeableConcept "A.1.8.2 - Document type" """Identifies the type of document at hand, e.g. Hospital discharge report."""
* documentMetadata.status 1..1 CodeableConcept "A.1.8.3 - Document status" """The status of the Hospital discharge report. E.g., preliminary, final."""
* documentMetadata.dateTime 1..1 dateTime "A.1.8.4 - Report date and time" """Date and time of the Hospital discharge report creation."""
* documentMetadata.title 1..1 string "A.1.8.5 - Document title" """Document title, fix value "Hospital discharge report"."""
* documentMetadata.custodian 1..1 Base "A.1.8.6 - Report custodian" """Organisation that is in charge of maintaining the report [this element will include organisation ID, name, address etc., as other elements describing organisations]."""
